...
fprintf('%s\n','SERIES ----------------------------------------------------------');
fprintf('%40s \t %s\n','Number of Series in This Scan',num2str(header.NumberofSources));
fprintf('%40s \t %s\n','Series Number',num2str(header.SourceIndex));
...
if isfield(header,'NumberofLesions')
    fprintf('%40s \t %s\n','Number of Lesions',num2str(header.NumberofLesions));
    if header.NumberofLesions > 0 
        breakpoints = [0,find(header.LesionPathologyArray==10),length(header.LesionPathologyArray)+1];
        for lesionIndex = 1:header.NumberofLesions
            fprintf('%40s \t %s\n',['Lesion',num2str(lesionIndex),' Pathology'],...
                header.LesionPathologyArray(breakpoints(lesionIndex)+1:breakpoints(lesionIndex+1)-1));
        end
        for lesionIndex = 1:header.NumberofLesions
            fprintf('%40s \t (%s, %s, %s)\n',['Lesion',num2str(lesionIndex),' Location (rho,phi,z)'],...
                num2str(header.LesionRadialDistanceArray(lesionIndex)),...
                num2str(header.LesionAngularPositionArray(lesionIndex)),...
                num2str(header.LesionAxialPositionArray(lesionIndex)));
        end
    end
end
